...
package dellemuse.serverdb.objectstorage;
...
import dellemuse.serverdb.SystemService;
...
public class ObjectStorageService extends BaseService implements SystemService {
    ...
    private synchronized void connect() {
        ...
        try {
            ...
            if (!this.client.existsBucket(ServerDBConstant.MEDIA_BUCKET)) {
                startupLogger.debug("Creating bucket -> " + ServerDBConstant.MEDIA_BUCKET);
                this.client.createBucket(ServerDBConstant.MEDIA_BUCKET);
            }

            if (!this.client.existsBucket(ServerDBConstant.THUMBNAIL_BUCKET)) {
                startupLogger.debug("Creating bucket -> " + ServerDBConstant.THUMBNAIL_BUCKET);
                this.client.createBucket(ServerDBConstant.THUMBNAIL_BUCKET);
            }
