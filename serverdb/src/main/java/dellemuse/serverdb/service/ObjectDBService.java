...
@Service
public class ObjectDBService extends BaseService implements SystemService {

    private static final Logger logger = Logger.getLogger(ObjectDBService.class.getName());

    public ObjectDBService(ServerDBSettings settings) {
        super(settings);
    }

    public Optional<DelleMuseObject> getObject(Class<?> clazz, Long id) {
    	
    	
    	ArtExhibitionDBService service = (ArtExhibitionDBService) ServiceLocator.getInstance().getBean(ArtExhibitionDBService.class);

    	Optional<ArtExhibition> o = service.findById(id);

    	
    	Optional<DelleMuseObject> r = Optional.of((DelleMuseObject) o.get());
