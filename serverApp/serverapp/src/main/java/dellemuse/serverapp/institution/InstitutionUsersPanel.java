...
public class InstitutionUsersPanel extends DBModelPanel<Institution> implements InternalPanel {

	private static final long serialVersionUID = 1L;

	static private Logger logger = Logger.getLogger(InstitutionUsersPanel.class.getName());
	...
	public InstitutionUsersPanel(String id, IModel<Institution> model) {
		super(id, model);
	...
	*/
	...
	protected void addListeners() {
		...
		add(new io.wktui.event.WicketEventListener<ObjectStateSelectEvent>() {
			...
			public void onEvent(ObjectStateSelectEvent event) {
				...
				load();
				event.getTarget().add(InstitutionUsersPanel.this.usersPanel);
				event.getTarget().add(InstitutionUsersPanel.this.listToolbarContainer);
			}
	...
	protected synchronized void load() {
		...
		if (!getModel().getObject().isDependencies()) {
			site = getInstitutionDBService().findWithDeps(getModel().getObject().getId()).get();
		}
		else
			site = getModel().getObject();
			
		getUserDBService().getInstitutionUsers( site ).forEach( u -> users.add( new ObjectModel<User>(u)) );
	}
	...
	private void addUsersPanel() {
		...
		this.usersPanel = new ListPanel<User>("users") {
			...
			protected Panel getListItemPanel(IModel<User> model) {
				...
				DelleMuseObjectListItemPanel<User> panel = new DelleMuseObjectListItemPanel<User>("row-element", model, getListPanelMode()) {
					...
					protected IModel<String> getObjectTitle() {
						return InstitutionUsersPanel.this.getObjectTitle(getModel());
					}
			...
			public List<IModel<User>> getItems() {
				return InstitutionUsersPanel.this.getUsers();
			}
