...
@Service
public class RoleSiteDBService extends DBService<RoleSite, Long> {

	private static final Logger logger = Logger.getLogger(RoleSiteDBService.class.getName());
	...
	@Transactional
	public void restore(RoleSite c, User restoredBy) {
	...
	public Optional<RoleSite> findWithDeps(Long id) {
		...
		Optional<RoleSite> o_i = super.findById(id);
	...
	@Transactional
	public Iterable<RoleSite> findAllSorted() {
		CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
		CriteriaQuery<RoleSite> cq = cb.createQuery(getEntityClass());
		Root<RoleSite> root = cq.from(getEntityClass());
		cq.orderBy(cb.asc(cb.lower(root.get("name"))));
	...
	@Transactional
	public Iterable<RoleSite> findAllSorted(ObjectState os) {
		CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
		CriteriaQuery<RoleSite> cq = cb.createQuery(getEntityClass());
		Root<RoleSite> root = cq.from(getEntityClass());
	...
	@Transactional
	public Iterable<RoleSite> findAllSorted(ObjectState os1, ObjectState os2) {
		CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
		CriteriaQuery<RoleSite> cq = cb.createQuery(getEntityClass());
		Root<RoleSite> root = cq.from(getEntityClass());
	...
	@Override
	protected Class<RoleSite> getEntityClass() {
		return RoleSite.class;
	}
