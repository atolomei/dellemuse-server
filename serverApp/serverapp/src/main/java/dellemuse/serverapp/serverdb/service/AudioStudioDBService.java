...
public class AudioStudioDBService extends DBService<AudioStudio, Long> {
	...
	/**
	@Transactional
	public Optional<GuideContent> getGuideContent( AudioStudio audioStudio) {
		
		Check.requireNonNullArgument(audioStudio, "AudioStudio is null");
		return Optional.of((GuideContent) audioStudio.getGuideContent());
	
	}

	
	@Transactional
	public Optional<ArtExhibition> getArtExhibition(AudioStudio audioStudio) {
		
		Check.requireNonNullArgument(audioStudio, "AudioStudio is null");
		
		AudioStudioBuild b=audioStudio.getParentOject();
		
		if (b instanceof Site) {
			return Optional.empty();
		}
		
		if (b instanceof ArtExhibition) {
			return Optional.of((ArtExhibition) b);
		}

		if (b instanceof ArtExhibitionGuide) {
			return Optional.of( ((ArtExhibitionGuide) b).getArtExhibition());
		}
		
		//if (b instanceof ArtExhibitionSectionGuide) {
		//	return ((ArtExhibitionSectionGuide) b).getArtExhibitionSection().getArtExhibition().getSite();
		//}

		
		if (b instanceof TranslationRecord) {

			MultiLanguageObject o = ((TranslationRecord) b).getParentObject();
		
			
			if (o instanceof ArtExhibitionGuide)
				Optional.of( ((ArtExhibitionGuide) o).getArtExhibition());
		
			if (o instanceof ArtExhibitionItem)
				Optional.of( ((ArtExhibitionItem) o).getArtExhibition() );
		
			if (o instanceof ArtExhibition)
				Optional.of( ((ArtExhibition) o));
		
		
			if (o instanceof ArtExhibitionSection)
				Optional.of( ((ArtExhibitionSection) o).getArtExhibition() );


			if (o instanceof GuideContent)
				Optional.of( ((GuideContent) o).getArtExhibitionItem().getArtExhibition());
	...
	public Optional<Site> getSite( AudioStudio audioStudio) {
		...
		if (b instanceof Site) {
			return Optional.of( (Site) b );
		}
		
		if (b instanceof ArtExhibition) {
			return Optional.of( ((ArtExhibition) b).getSite() );
		}

		if (b instanceof ArtExhibitionGuide) {
			return Optional.of( ((ArtExhibitionGuide) b).getArtExhibition().getSite());
		}
		...
		if (b instanceof GuideContent) {
			return  Optional.of( ((GuideContent) b).getArtExhibitionItem().getArtExhibition().getSite() );
		}
		...
		if (b instanceof TranslationRecord) {
			...
			if (o instanceof Site)
				return Optional.of( (Site) o);
			
			if (o instanceof ArtExhibitionGuide)
				Optional.of(   ((ArtExhibitionGuide) o).getArtExhibition().getSite()   );
		
			if (o instanceof ArtExhibitionItem)
				Optional.of( ((ArtExhibitionItem) o).getArtExhibition().getSite() );
		
			if (o instanceof ArtExhibition)
				Optional.of( ((ArtExhibition) o).getSite() );
		
		
			if (o instanceof ArtExhibitionSection)
				Optional.of( ((ArtExhibitionSection) o).getArtExhibition().getSite() );

			if (o instanceof ArtWork)
				Optional.of( ((ArtWork) o).getSite() );

			if (o instanceof GuideContent)
				Optional.of( ((GuideContent) o).getArtExhibitionItem().getArtExhibition().getSite() );
	...
	*/
	...
	@Override
	public AudioStudio create(String name, User createdBy) {
		throw new RuntimeException("can not create without owner object");
	}
	...
	@Transactional
	public void delete(AudioStudio o) {
		this.delete(o.getId()); 
	}
	...
	public Optional<AudioStudioParentObject> findParentObjectWithDeps(AudioStudio audioStudio) {
		...
		logger.error("nor parent -> " + b.getClass().getSimpleName());
