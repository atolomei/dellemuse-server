...
public class FloorRecordEditor extends ObjectRecordEditor<Floor, FloorRecord> {

	private static final long serialVersionUID = 1L;

	static private Logger logger = Logger.getLogger(FloorRecordEditor.class.getName());
	...
	public FloorRecordEditor(String id, IModel<Floor> sourceModel, IModel<FloorRecord> recordModel) {
	...
	public void onInitialize() {
		...
		Optional<FloorRecord> o = ((FloorRecordDBService) ServiceLocator.getInstance().getBean(FloorRecordDBService.class))
			.findWithDeps(getModel().getObject().getId());
		if (o.isPresent())
			setModel(new ObjectModel<FloorRecord>(o.get()));

		// audioAccessible
		//infoAccessibleField = new TextAreaField<String>(
		//	"infoAccessible",
	//		new PropertyModel<String>(getModel(), "infoAccessible"),
	//		getLabel("info-accessible"), 4);

		/**
		audioAccessibleField = new FileUploadSimpleField<Resource>("audioAccessible", getAudioAccessibleModel(), getLabel("audio-accessible")) {
			private static final long serialVersionUID = 1L;

			@Override
			protected boolean processFileUploads(List<FileUpload> uploads) {
				return FloorRecordEditor.this.processAudioAccessibleUpload(uploads);
			}

			@Override
			protected void onRemove(AjaxRequestTarget target) {
				FloorRecordEditor.this.onAudioAccessibleRemove(target);
			}
			...
			protected String getAudioSrc() {
				if (getAudioAccessibleModel() == null || getAudioAccessibleModel().getObject() == null)
					return null;
				try {
					return getObjectStorageService().getPublicUrl(getAudioAccessibleModel().getObject());
				} catch (Exception e) {
					logger.error(e);
					return null;
				}
			...
			public String getAudioMetadata() {
				if (getAudioAccessibleModel() != null && getAudioAccessibleModel().getObject() != null)
					return FloorRecordEditor.this.getAudioMeta(getAudioAccessibleModel().getObject());
				return null;
			}
		};
	...
	protected void setUpModel() {
		...
		FloorRecord fr = getModel().getObject();
		if (fr.getAudioAccessible() != null) {
			Optional<Resource> o = getResourceDBService().findWithDeps(fr.getAudioAccessible().getId());
			...
			for (FileUpload upload : uploads) {
				try {
					...
					Resource resource = createAndUploadFile(
						...
						upload.getClientFileName(), upload.getSize(), false);
	...
	public void onDetach() {
		super.onDetach();
		if (audioAccessibleModel != null) audioAccessibleModel.detach();
		if (siteModel != null)            siteModel.detach();
	}

	public IModel<Resource> getAudioAccessibleModel() { return audioAccessibleModel; }
	public void setAudioAccessibleModel(IModel<Resource> model) { this.audioAccessibleModel = model; }

	public IModel<Site> getSourceSiteModel() { return siteModel; }
	public void setSourceSiteModel(IModel<Site> model) { this.siteModel = model; }
}
