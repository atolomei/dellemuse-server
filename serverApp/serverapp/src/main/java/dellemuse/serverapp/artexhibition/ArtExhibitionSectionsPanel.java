...
public class ArtExhibitionSectionsPanel extends DBModelPanel<ArtExhibition> implements InternalPanel {
	 ...
	 * 
	 * this.add = new AjaxLink<Void>("add") { private static final long
	 * serialVersionUID = 1L;
	 * 
	 * @Override public void onClick(AjaxRequestTarget target) { setState(
	 *           FormState.EDIT); target.add(addContainerButtons);
	 *           //target.add(addContainer);
	 * 
	 *           }
	 * 
	 *           public boolean isVisible() { return getState()==FormState.VIEW; }
	 *           };
	 * 
	 ...
	 *           private static final long serialVersionUID = 1L;
	 * @Override public void onClick(AjaxRequestTarget target) { setState(
	 *           FormState.VIEW); target.add(addContainerButtons); } public boolean
	 *           isVisible() { return getState()==FormState.EDIT; } };
	 * 
	...
	private void addSections() {
		...
		this.sectionsPanel = new ListPanel<ArtExhibitionSection>("items") {
			...
			/**
			 * protected List<IModel<ArtExhibitionSection>>
			 * filter(List<IModel<ArtExhibitionSection>> initialList, String filter) {
			 * List<IModel<ArtExhibitionSection>> list = new
			 * ArrayList<IModel<ArtExhibitionSection>>(); final String str =
			 * filter.trim().toLowerCase(); initialList.forEach(s -> { if
			 * (s.getObject().getDisplayname().toLowerCase().contains(str)) { list.add(s); }
			 * }); return list; }
			 **/

			@Override
			public IModel<String> getItemLabel(IModel<ArtExhibitionSection> model) {
				return ArtExhibitionSectionsPanel.this.getObjectTitle(model.getObject());
			}
			...
			protected Panel getListItemPanel(IModel<ArtExhibitionSection> model) {
				DelleMuseObjectListItemPanel<ArtExhibitionSection> panel = new DelleMuseObjectListItemPanel<ArtExhibitionSection>("row-element", model, getListPanelMode()) {
					private static final long serialVersionUID = 1L;
					...
					public void onClick() {
						// setResponsePage(new
						// ArtExhibitionSectionPage(getModel(),ArtExhibitionSectionsPanel.this.getItems()));
					}
