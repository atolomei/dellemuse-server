...
public abstract class BaseSiteSearcherPanel2 extends DBModelPanel<Site> implements InternalPanel {

	private static final long serialVersionUID = 1L;

	@SuppressWarnings("unused")
	static private Logger logger = Logger.getLogger(BaseSiteSearcherPanel2.class.getName());
	...
	private List<IModel<ArtExhibitionGuide>> g_list;
	
	
	private ListPanel<GuideContent> itemsPanel;

	private WebMarkupContainer itemsGuideContensContainer;
	private WebMarkupContainer itemsArtExhibitionGuidesContainer;
	...
	public BaseSiteSearcherPanel2(String id, IModel<Site> model) {
		super(id, model);
	...
	public void onInitialize() {
		...
		itemsGuideContensContainer = new WebMarkupContainer("itemsGuideContentContainer");
		add(itemsGuideContensContainer);
		itemsGuideContensContainer.setVisible(false);
		itemsGuideContensContainer.setOutputMarkupId(true);

		
		itemsArtExhibitionGuidesContainer = new WebMarkupContainer("itemsArtExhibitionGuideContainer");
		add(itemsArtExhibitionGuidesContainer);
		itemsArtExhibitionGuidesContainer.setVisible(false);
		itemsArtExhibitionGuidesContainer.setOutputMarkupId(true);
	...
	protected Panel getObjectListItemExpandedPanel(IModel<GuideContent> model, ListPanelMode mode) {
		...
		return new ObjectListItemExpandedPanel<GuideContent>("expanded-panel", model, mode) {
			...
			protected IModel<String> getInfo() {
				return BaseSiteSearcherPanel2.this.getObjectInfo(getModel());
			}

			@Override
			protected IModel<String> getObjectSubtitle() {
				return BaseSiteSearcherPanel2.this.getObjectSubtitle(getModel());
			}

			@Override
			protected String getImageSrc() {
				return BaseSiteSearcherPanel2.this.getObjectImageSrc(getModel());
			}
		};
	}
	...
	protected void addForm() {
		...
		SubmitButton<Void> submit = new SubmitButton<Void>("submit") {
			...
			@Override
			protected void onSubmit(AjaxRequestTarget target) {
				BaseSiteSearcherPanel2.this.onSubmit(target);
			}

			public String getIcon() {
				return BaseSiteSearcherPanel2.this.getIcon();
				 
			}

			public String getIconStyle() {
				return BaseSiteSearcherPanel2.this.getIconStyle();
			}
			...
			protected String getSaveCss() {
				return BaseSiteSearcherPanel2.this.getSaveCss();
			}
		};
	...
	protected void addItems() {
		...
		this.itemsPanel = new ListPanel<GuideContent>("items") {
			...
			public IModel<String> getItemLabel(IModel<GuideContent> model) {
				return BaseSiteSearcherPanel2.this.getObjectTitle(model);
			}

			@Override
			protected WebMarkupContainer getListItemExpandedPanel(IModel<GuideContent> model, ListPanelMode mode) {
				return BaseSiteSearcherPanel2.this.getObjectListItemExpandedPanel(model, mode);
			}
			...
			protected Panel getListItemPanel(IModel<GuideContent> model) {
				...
				DelleMuseObjectListItemPanel<GuideContent> panel = new DelleMuseObjectListItemPanel<GuideContent>("row-element", model, getListPanelMode()) {
					...
					protected IModel<String> getObjectTitle() {
						return BaseSiteSearcherPanel2.this.getObjectTitle(getModel());
					}
					...
					@Override
					public void onClick() {
						BaseSiteSearcherPanel2.this.onClick(getModel());
					}
					...
					protected WebMarkupContainer getObjectMenu() {
						return BaseSiteSearcherPanel2.this.getMenu(getModel());
					}
			...
			public List<IModel<GuideContent>> getItems() {
				return BaseSiteSearcherPanel2.this.getItems();
			}
		};
