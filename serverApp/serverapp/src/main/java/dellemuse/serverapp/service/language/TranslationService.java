...
public class TranslationService extends BaseService {
	...
	public boolean translateSite(Site src, SiteRecord dest) {
		...
		try {
			...
			if (requiresTranslation(name, dest.getName(), dest.getNameHash())) {
				...
				Translation translation = translate.translate(name, Translate.TranslateOption.format("text"), Translate.TranslateOption.sourceLanguage(src.getMasterLanguage()), Translate.TranslateOption.targetLanguage(dest.getLanguage()));
			...
			if (requiresTranslation(subtitle, dest.getSubtitle(), dest.getSubtitleHash())) {
				...
				Translation translation = translate.translate(subtitle, Translate.TranslateOption.format("text"), Translate.TranslateOption.sourceLanguage(src.getMasterLanguage()),
			...
			if (requiresTranslation(info, dest.getInfo(), dest.getInfoHash())) {
				...
				Translation translation = translate.translate(info, Translate.TranslateOption.format("text"), Translate.TranslateOption.sourceLanguage(src.getMasterLanguage()), Translate.TranslateOption.targetLanguage(dest.getLanguage()));
			...
			if (requiresTranslation(intro, dest.getIntro(), dest.getIntroHash())) {
				...
				Translation translation = translate.translate(intro, Translate.TranslateOption.format("text"), Translate.TranslateOption.sourceLanguage(src.getMasterLanguage()), Translate.TranslateOption.targetLanguage(dest.getLanguage()));
			...
			if (requiresTranslation(address, dest.getAddress(), dest.getAddressHash())) {
				...
				Translation translation = translate.translate(address, Translate.TranslateOption.format("text"), Translate.TranslateOption.sourceLanguage(src.getMasterLanguage()),
						Translate.TranslateOption.targetLanguage(dest.getLanguage()));
			...
			if (requiresTranslation(opens, dest.getAddress(), dest.getOpensHash())) {
				...
				Translation translation = translate.translate(opens, Translate.TranslateOption.format("text"), Translate.TranslateOption.sourceLanguage(src.getMasterLanguage()), Translate.TranslateOption.targetLanguage(dest.getLanguage()));
			...
			if (requiresTranslation(spec, dest.getSpec(), dest.getSpecHash())) {
				...
				Translation translation = translate.translate(spec, Translate.TranslateOption.format("text"), Translate.TranslateOption.sourceLanguage(src.getMasterLanguage()), Translate.TranslateOption.targetLanguage(dest.getLanguage()));
		...
		try {
			...
			if (requiresTranslation(name, dest.getName(), dest.getNameHash())) {
				...
				if (name == null) {
					...
					Translation translation = translate.translate(name, Translate.TranslateOption.format("text"), Translate.TranslateOption.sourceLanguage(src.getMasterLanguage()),
							Translate.TranslateOption.targetLanguage(dest.getLanguage()));
			...
			if (requiresTranslation(info, dest.getInfo(), dest.getInfoHash())) {
				...
				if (info == null) {
					...
					Translation translation = translate.translate(info, Translate.TranslateOption.format("text"), Translate.TranslateOption.sourceLanguage(src.getMasterLanguage()),
							Translate.TranslateOption.targetLanguage(dest.getLanguage()));
			...
			if (requiresTranslation(intro, dest.getIntro(), dest.getIntroHash())) {
				...
				if (intro == null) {
					...
					Translation translation = translate.translate(intro, Translate.TranslateOption.format("text"), Translate.TranslateOption.sourceLanguage(src.getMasterLanguage()),
							Translate.TranslateOption.targetLanguage(dest.getLanguage()));
	...
	private void initializeTranslateClient() {
		...
		if (translate == null) {
			 ...
			 * } catch (Exception e) {
			 * 
			 * serviceEnabled.set(false); startupLogger.error(e, "Failed to initialize
			...
			startupLogger.info("Starting connection to Google Translate API");
			...
			try {
				...
				if (filePath != null && !filePath.trim().isEmpty()) {
					...
					if (!file.exists())
						throw new RuntimeException("Google credentials file does not exist -> " + filePath);

					if (!file.canRead())
						throw new RuntimeException("Google credentials file does not have read permission -> " + filePath);

					// 1. Si definiste una ruta (ideal para Producción o pruebas específicas)
					startupLogger.info("starting Google Translate using Google Credentianls -> " + filePath);

					try (FileInputStream credentialsStream = new FileInputStream(filePath)) {
						GoogleCredentials credentials = GoogleCredentials.fromStream(credentialsStream);
						optionsBuilder.setCredentials(credentials);
					}

				} else {
					// 2. Si NO definiste ruta (ideal para Desarrollo local en Mac)
					// No hacemos nada, la librería usará automáticamente Application Default
					// Credentials
					// (lo que logueaste con gcloud)
					startupLogger.info("Using Application Default Credentials (gcloud/env)");
				}

				this.translate = optionsBuilder.build().getService();

				serviceEnabled.set(true);
				startupLogger.info("Google Translate client -> ok");
				...
				if (filePath != null && !filePath.trim().isEmpty())
					startupLogger.error("Credentials file -> " + filePath);
				startupLogger.error(ServerConstant.SEPARATOR);
