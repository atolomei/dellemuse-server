...
public class ElevenLabsBaseRecord extends JsonObject {
	 ...
	 * the same result. Determinism is not guaranteed. Must be integer between 0 and
	 * 4294967295.
	 */
	public Integer seed;
	 ...
	 * improve the speech's continuity when concatenating together multiple
	 * generations or to influence the speech's continuity in the current
	 * generation.
	 */
	 ...
	 * generation. Can be used to improve the speech's continuity when splitting up
	 * a large task into multiple requests. The results will be best when the same
	 * model is used across the generations. In case both previous_text and
	 * previous_request_ids is send, previous_text will be ignored. A maximum of 3
	 ...
	 * when regenerating a sample that has had some audio quality issues. For
	 * example, if you have generated 3 speech clips, and you want to improve clip
	 * 2, passing the request id of clip 3 as a next_request_id (and that of clip 1
	 * as a previous_request_id) will help maintain natural flow in the combined
	 * speech. The results will be best when the same model is used across the
	 * generations. In case both next_text and next_request_ids is send, next_text
	 ...
	 * and 'off'. When set to 'auto', the system will automatically decide whether
	 * to apply text normalization (e.g., spelling out numbers). With 'on', text
	 * normalization will always be applied, while with 'off', it will be skipped.
	 * For 'eleven_turbo_v2_5' and 'eleven_flash_v2_5' models, text normalization
	 * can only be enabled with Enterprise plans.
	 */
	public TextNormalization apply_text_normalization;

	/**
	 * voice_settings": { "stability": 0.5, "style": 0, "speed": 1,
	 * "use_speaker_boost": true, "similarity_boost": 0.75 }
	 **/
	 ...
	 * computational resources and might increase latency if set to anything other
	 * than 0.
	 */
	public Double style;
	 ...
	 * values less than 1.0 slow down the speech, and values greater than 1.0 speed
	 * it up
	 */
	public Double speed;
